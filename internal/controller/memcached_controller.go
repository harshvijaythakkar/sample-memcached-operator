...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !controllerutil.ContainsFinalizer(memcached, memcachedFinalizer) {
		...
		if ok := controllerutil.AddFinalizer(memcached, memcachedFinalizer); !ok {
			logger.Error(nil, "Failed to add finalizer into the custom resource")
	...
	if isMemcachedMarkedToBeDeleted {
		if controllerutil.ContainsFinalizer(memcached, memcachedFinalizer) {
			...
			meta.SetStatusCondition(&memcached.Status.Conditions, metav1.Condition{Type: typeDegradedMemcached, Status: metav1.ConditionTrue, Reason: "Finalizing", Message: fmt.Sprintf("Finalizer operations for custom resource %s name were successfully accomplished", memcached.Name)})
			if err := r.Status().Update(ctx, memcached); err != nil {
				logger.Error(err, "Failed to update Memcached status")
				return ctrl.Result{}, err
			}
			...
			if ok := controllerutil.RemoveFinalizer(memcached, memcachedFinalizer); !ok {
				logger.Error(nil, "Failed to remove finalizer for Memcached")
				return ctrl.Result{Requeue: true}, nil
			}

			if err := r.Update(ctx, memcached); err != nil {
				logger.Error(err, "Failed to remove finalizer for Memcached")
				return ctrl.Result{}, err
			}
		}
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			logger.Error(err, "Failed to define new Deployment resource for Memcached")

			// The following implementation will update the status
			meta.SetStatusCondition(&memcached.Status.Conditions, metav1.Condition{Type: typeAvailableMemcached, Status: metav1.ConditionFalse, Reason: "Reconciling", Message: fmt.Sprintf("Failed to create Deployment for the custom resource (%s): (%s)", memcached.Name, err)})
			if err := r.Status().Update(ctx, memcached); err != nil {
				logger.Error(err, "Failed to define new Deployment resource for Memcached")
				return ctrl.Result{}, err
			}
	...
	if *found.Spec.Replicas != size {
		logger.Info("Updating deployment size")
		...
		if err := r.Update(ctx, found); err != nil {
			...
			// The following implementation will update the status
			meta.SetStatusCondition(&memcached.Status.Conditions, metav1.Condition{Type: typeAvailableMemcached, Status: metav1.ConditionFalse, Reason: "Resizing", Message: fmt.Sprintf("Failed to update the size for the custom resource (%s): (%s)", memcached.Name, err)})
			if err := r.Status().Update(ctx, memcached); err != nil {
				logger.Error(err, "Failed to update Memcached status")
				return ctrl.Result{}, err
			}
...
func (r *MemcachedReconciler) deploymentForMemcached(memcached *cachev1alpha1.Memcached) (*appsv1.Deployment, error) {
	ls := labelsForMemcached(memcached)
...
func labelsForMemcached(memcached *cachev1alpha1.Memcached) map[string]string {
	image := strings.Split(memcached.Spec.ContainerImage, ":")[1]
